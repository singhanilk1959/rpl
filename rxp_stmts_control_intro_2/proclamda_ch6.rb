#!/usr/bin/env ruby
puts %q^
A method is a named block of paramaterized code associated with one or more objects.
The value of last expression evaluated in the method becomes the value of the method invocation expression.

Ruby is a purely object oriented language and all methods are associated with atleast one object.
So called global functions like factorial below will be defined as the private methods of Object class
and and is called without receiver as per private functions syntax.

Methods are fundamental part of Ruby's syntax. It is, however, possible to
obtain a Method object representing the method and we can invoke methods indiretly through Method objects.

Blocks are also paramaterized codes without any names and can only be invoked indirectly through an iterator
method.

In Ruby programs, it is possible to create an object that represents a block.
A Proc represnts a block. There are two varieties of Proc objects: procs and lambdas.

An important feature of procs and lambdas is that they are clsoures: they retain access
to local variables that were in scope when they were defined, even when proc or lambda is
called from different scope.
