...
Methods are modified with a technique called alias chaining. It works like this:

1. First create an alias for the method to be modified. This alias provides a 
   name for the unmodified version of the method.

2.Next define a new version of the method. The new version should call the
  unmodified version through the alias, but it can add whatever functionality 
  is needed before and after it does that.   
...
This classtrace module keeps track of all the files loaded and all classes definedin a program. When program exits, it prints a report.
************************
^
gets
puts %q^
...
def require(file)
  ...
  original_require(file)                # Invoke the original method
					# caller defined in Kernel provides
					# stack trace (ri caller)
end
def load(*args)
