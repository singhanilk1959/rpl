...
gets
print %^
Ruby supports multiline comment known as embedded document.
It can be used to include long blocks of documentation within a program, 
or to embed source code of another language.
It is typical to follow =begin with an identifier that indicates which tool the comment is intended for.
...
gets
print %^
Ruby programs can include embedded API documentation as specially formatted comments that precede method, class, and module definitions. You can use ri to browse these documentations. The rdoc tool extracts documentation comments from ruby source.
...
=begin    =end      __END__
*************
=begin and =end at the begining of a line delimit multiine comments
__END__ denotes the end of the program and begining of data section

Treat these keywords as reserved and don't use them as variable names or method names
^
gets
print %^
**************
...
print %^
**************
you can use semicolons to terminate statements in Ruby, but this is only
required if you use more than one statements on the same line.
Convention dictates that semicolons be omitted elsewhere.

Without semicolns, the Ruby Interpreter must figure out on its own where
statements end.
---------------------------
...
print %^
**************
Ruby allows the paranthesis around method invocations to be omitted in certain circumstances.
This allows ruby methods to be used as if they were statements, which is an important part of Ruby's elegance.

f(3+2)+1        # f(5)+1
f (3+2)+1       # f(6)

The solution is that never put a space between a method name and opening paranthisis.

Always run Ruby interpreter with -w option which issues warning.
---------------------------
